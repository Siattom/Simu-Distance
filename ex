const RoadIO = (function() {
    ...
    function locationMarker(map, location, roadClient) {
     ...
     marker.addListener('mouseout', function() {
      ...
      marker.addListener('click', async function() {
        console.log(location, "j'ai clic")
    ...
    function renderSidebar(location) {
      console.log(location.floor_level)

      const lat = location.position.lat;
      const lng = location.position.lng;
      var name = '';
    
      // Effectuer une requête à l'API Geocoding de Google Maps
      fetch(`https://maps.googleapis.com/maps/api/geocode/json?latlng=${lat},${lng}&key=AIzaSyA5bqh8cJjgLX6-THZEgmBq61jf8vCxa9Q`)
      ...
      content.push(`<a class="directions" href="https://maps.google.com/maps?daddr=${location.position[0]},${location.position[1]}" target="_blank" style="margin: 0"><span>${carSvg} Directions</span></a>`);
      content.push('</div>');
      content.push('<div class="divider"></div>');
      console.log('ici pour assurer que le htmlest ok')
      return content.join(' ');
    }
