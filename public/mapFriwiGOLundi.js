...
async function fetchNearestChargePoints(latitude, longitude, selectedChargeType) {
  ...
  // Ajuster les paramètres pour un périmètre de 30 km
  const params = {
      zoom: 14,
  ...
  try {
      ...
      if (data && data.locations && data.locations.length > 0) {
          ...
          for (let chargePoint of data.locations) {
              ...
              if (selectedChargeType === 'normal') {
                  if (details.maxPower >= 3 && details.maxPower <= 22 && details.status === 'AVAILABLE') {
                      etat = true;
                  }
              ...
              } else if (selectedChargeType === 'all') {
...
function reCalcIti(insufficientChargePoint, prevCharge, result, destinationCoordinates) {
  ...
  console.log(insufficientChargePoint, 'oui oui')
...
// -------------------------------------------------------------------------------------------------------------------
// ------------------------ CONDITION PRISE DANS FETCHNEARESTCHARGEPOINT ---------------------------------------------
// -------------------------------------------------------------------------------------------------------------------
        // Vérifier les critères pour chaque point de recharge trié
        for (let chargePoint of data.locations) {
          const details = await roadClient.fetchLocationDetails({
