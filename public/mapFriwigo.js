const RoadIO = (function() {
  ...
  function labelForStatus(status) {
    ...
    if (status === 'NOT_AVAILABLE') return 'Non disponible';
    if (status === 'UNAVAILABLE') return 'Non disponible';
    if (status === 'SUSPENDEDEV') return 'EV suspendu';
  ...
  function locationMarker(map, location, roadClient) {
    ...
    switch (location.status) {
      ...
      case StatusUnknown:
        ...
        break;
      default:
        throw `Invalid statut: ${location.status}`;
    }
  ...
  function translatePriceComponent(priceComponent, currency) {
    switch (priceComponent.type) {
      case 'FLAT':
        return `Frais de départ ${formatCurrency(priceComponent.price * 1.21, {
          currency,
        })} TVA incluse`;
      case 'TIME':
        return `Tarif horaire ${formatCurrency(priceComponent.price * 1.21, {
            currency,
          })} TVA incluse par heure`;
        case 'ENERGY':
          return `Coût de l'énergie ${formatCurrency(priceComponent.price * 1.21, {
            currency,
          })} TVA incluse par kWh`;
        default:
          return '';
  ...
  function renderConnectorTariff(tariff, unreliableTariffs, customNote) {
    ...
    const unreliableTariffsNote = "Cet opérateur pourrait ne pas envoyer des tarifs précis. Veuillez vérifier les informations tarifaires sur place ou auprès du service d'assistance de l'opérateur."
    const inaccurateTariffsNote = 'Les tarifs indiqués ici ont été envoyés à E-Flux par un partenaire d\'itinérance. Leur exactitude peut varier en fonction du fournisseur.'

    content.push('<div class="italicP" style="font-size: 12px">');
      if (hasPriceComponents) {
        ...
        content.push('<div>Tarif non disponible</div>');
      }
    ...
    content.push('<div>');
      ...
      content.push('<table class="inverted">');
        content.push('<tbody>');
          ...
          if (evse.maxPower) {
            content.push('<tr>');
              content.push('<td>Puissance max.</td>');
  ...
  function renderSidebar(location) {
    ...
    content.push('<div style="padding-left: 1em; padding-right: 1em" class="sidebar-container">');
      ...
      content.push('<div class="divider"></div>');
      content.push(`<p>Mise à jour <b>${location.updatedAt && new Date(location.updatedAt).toDateString()}</b> de <b>${sanitize(location.source)}</b></p>`)
      content.push('<div class="divider"></div>');
...
function verifWaypointsCalcNewRoute(verifWaypoints, selectedChargeType, result, prevCharge, destinationCoordinates, map) {
  ...
  function calcRoute() {
    ...
    // Crée une instance personnalisée de DirectionsRenderer
    directionsDisplay = new google.maps.DirectionsRenderer({
        map: RoadIO.getMap(),
        suppressMarkers: true // Masquer les marqueurs par défaut
    });
    ...
    directionsService.route(request, function (result, status) {
        if (status == google.maps.DirectionsStatus.OK) {
            ...
            if (prevCharge <= distance) {
                ...
                // Ajouter des waypoints pour les bornes de recharge
                calcNewRoute(result, prevCharge, result.routes[0].legs[0].end_location, insufficientChargePoint);
            } else {
...
function addCustomMarkers(result) {
  ...
  const endIcon = window.btoa(`
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 240 240">
    <circle cx="120" cy="120" opacity="1" r="50" fill="#0000CC" stroke="white" stroke-width="12" />
  </svg>`);
  const waypointIcon = window.btoa(`
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 240 240">
    <circle cx="120" cy="120" opacity="1" r="50" fill="#006600" stroke="white" stroke-width="12" />
  </svg>`);

  var startSvg = {
    url: `data:image/svg+xml;base64,${startIcon}`, // URL de l'icône personnalisée
  ...
  var endSvg = {
    url: `data:image/svg+xml;base64,${endIcon}`, // URL de l'icône personnalisée
    scaledSize: new google.maps.Size(32, 32), // Taille de l'icône
    origin: new google.maps.Point(0, 0), // Origine de l'icône (coin supérieur gauche)
    anchor: new google.maps.Point(16, 32) // Point d'ancrage de l'icône (centre inférieur)
  };
  ...
  // Créer et ajouter des marqueurs pour les waypoints
  const waypoints = result.request.waypoints;
  if (waypoints) {
    waypoints.forEach(function(waypoint, index) {
      var waypointMarker = new google.maps.Marker({
        position: result.routes[0].legs[index + 1].start_location, // Correct position of waypoint
        map: RoadIO.getMap(),
        icon: waypointSvg,
        title: 'Waypoint'
      });
...
if (selectedLocation === locationData) {
          ...
          console.error('No details found for the selected location');
        }
      } catch (error) {
        console.error('Error fetching location details:', error);
      }
        ...
        if (data.status === 'OK' && data.results.length > 0) {
            ...
            //console.log(addressData, 'addressData')
                ...
                if (!isAddressAlreadyAdded) {
                  waypoints.push({
                    location: addressData,// ICI je n'est pas accès a addressData
                    stopover: true
                  });
                }
            ...
            directionsService.route(request, function(result, updatedStatus) {
                if (updatedStatus == google.maps.DirectionsStatus.OK) {
                  ...
                  // Ajouter les marqueurs de bornes de recharge
                  addCustomMarkers(result);
...
stationElements.forEach(function(element) {
  ...
  function renderEvse(location, evse) {
    ...
    var etat = sanitize(startCase((evse.status || '').toLowerCase()));
    var etatVerif = '';
    ...
    if(etat === 'Charging'){
      ...
      etatVerif = 'Disponible';

    } else if(etat === 'Not_Available'){

      etatVerif = 'Non disponible';

    } else if(etat === 'Unavailable'){

      etatVerif = 'Non disponible';

    } else if(etat === 'SUSPENDEDEV'){

      etatVerif = 'EV suspendu';
  ...
  function renderConnectorTariff(tariff, unreliableTariffs, customNote) {
    ...
    content.push('<div class="italicP" style="font-size: 12px">');
      if (hasPriceComponents) {
        ...
        content.push('<div>Pas de tarif disponible</div>');
      }
      for (const element of elements) {
        const price_components = element.price_components || [];
        for (const component of price_components) {
  ...
  function formatConnectorType(value) {
    ...
    if (value === 'IEC_62196_T3C') return 'Type 3C';
    if (value === 'CHADEMO') return 'CHAdeMO';
    if (value === 'DOMESTIC_F') return 'Domestique F';
    if (value === 'DOMESTIC_E') return 'Domestique E';
    if (value === 'DOMESTIC_B') return 'Domestique B';
    if (value === 'CATARC') return 'Catarc';
    return value;
  };
